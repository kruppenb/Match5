...
export class GameScene extends Phaser.Scene {
  ...
  private drawTileShape(graphics: Phaser.GameObjects.Graphics, type: string, color: number, halfSize: number): void {
    ...
    switch (type) {
      case 'red': // Gem/Ruby
        this.drawGem(graphics, color, halfSize);
        break;
      case 'blue': // Water droplet
        this.drawDroplet(graphics, color, halfSize);
        break;
      case 'green': // Leaf
        this.drawLeaf(graphics, color, halfSize);
        break;
      case 'yellow': // Star
        this.drawStar(graphics, halfSize, 5);
        break;
      case 'purple': // Moon
        this.drawMoon(graphics, color, halfSize);
        break;
      case 'orange': // Sun
        this.drawSun(graphics, color, halfSize);
        break;
      default:
        // Circle fallback
  ...
  private drawPowerupShape(graphics: Phaser.GameObjects.Graphics, powerupType: string, color: number, halfSize: number): void {
    switch (powerupType) {
      case 'rocket_h': // Horizontal rocket
        this.drawRocket(graphics, color, halfSize, 'horizontal');
        break;
      case 'rocket_v': // Vertical rocket
        this.drawRocket(graphics, color, halfSize, 'vertical');
        break;
      case 'bomb': // Bomb
        this.drawBomb(graphics, color, halfSize);
        break;
  ...
  private drawGem(graphics: Phaser.GameObjects.Graphics, color: number, halfSize: number): void {
    ...
    graphics.fillStyle(this.lightenColor(color, 0.3), 1);
    graphics.beginPath();
    graphics.moveTo(0, -s);
  ...
  private drawDroplet(graphics: Phaser.GameObjects.Graphics, color: number, halfSize: number): void {
    const s = halfSize * 0.85;
    ...
    graphics.fillStyle(this.lightenColor(color, 0.25), 1);
    graphics.beginPath();
  ...
  private drawLeaf(graphics: Phaser.GameObjects.Graphics, color: number, halfSize: number): void {
    const s = halfSize * 0.85;
    ...
    // Center vein
    graphics.lineStyle(3, this.darkenColor(color, 0.2), 1);
    graphics.lineBetween(0, -s * 0.7, 0, s * 0.7);

    // Side veins
    graphics.lineStyle(2, this.darkenColor(color, 0.15), 1);
  ...
  private drawMoon(graphics: Phaser.GameObjects.Graphics, color: number, halfSize: number): void {
    const s = halfSize * 0.85;
    ...
    this.drawMiniStar(graphics, s * 0.5, -s * 0.5, s * 0.12);
    this.drawMiniStar(graphics, s * 0.7, s * 0.1, s * 0.08);
    this.drawMiniStar(graphics, s * 0.35, s * 0.55, s * 0.1);

    // Moon surface details
    graphics.fillStyle(this.darkenColor(color, 0.1), 0.5);
    graphics.fillCircle(-s * 0.3, -s * 0.2, s * 0.12);
    graphics.fillCircle(-s * 0.5, s * 0.3, s * 0.08);
    graphics.fillCircle(-s * 0.15, s * 0.4, s * 0.1);
  }

  private drawMiniStar(graphics: Phaser.GameObjects.Graphics, x: number, y: number, size: number): void {
    const points = 4;
  ...
  private drawSun(graphics: Phaser.GameObjects.Graphics, color: number, halfSize: number): void {
    const s = halfSize * 0.85;
    const rays = 8;
  ...
  private drawRocket(graphics: Phaser.GameObjects.Graphics, color: number, halfSize: number, direction: 'horizontal' | 'vertical'): void {
    ...
    if (direction === 'horizontal') {
      ...
      // Fins
      graphics.fillStyle(this.darkenColor(color, 0.2), 1);
      // Top fin
  ...
  private drawBomb(graphics: Phaser.GameObjects.Graphics, color: number, halfSize: number): void {
    ...
    this.drawMiniStar(graphics, s * 0.35, -s * 1.45, s * 0.08);
    this.drawMiniStar(graphics, s * 0.05, -s * 1.5, s * 0.06);
    this.drawMiniStar(graphics, s * 0.3, -s * 1.2, s * 0.05);
  ...
  private drawColorBomb(graphics: Phaser.GameObjects.Graphics, halfSize: number): void {
    ...
    this.drawMiniStar(graphics, 0, 0, s * 0.2);

    // Outer sparkles
    graphics.fillStyle(0xffffff, 0.8);
    for (let i = 0; i < 6; i++) {
      const angle = (i * 2 * Math.PI) / 6;
      this.drawMiniStar(graphics, Math.cos(angle) * s * 0.85, Math.sin(angle) * s * 0.85, s * 0.1);
    }
  }
  ...
  private drawPropeller(graphics: Phaser.GameObjects.Graphics, color: number, halfSize: number): void {
    ...
    graphics.fillStyle(this.lightenColor(color, 0.2), 1);
    graphics.beginPath();
    graphics.moveTo(0, -s * 0.9);
    ...
    // Sparkle at nose
    graphics.fillStyle(0xffffff, 0.8);
    this.drawMiniStar(graphics, 0, -s * 0.7, s * 0.12);
  }
